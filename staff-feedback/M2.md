...
EDIT: The version of the Figma that we looked was not the most recent one due to the versioning, here is the corrected feedbacK. Your Figma looks good and is mostly up to date with your app, but here are a few points that you could improve:
- Use more realistic placeholders in the Mock-Up -> instead of writing "Title" in the profile page where you display the activities, you could provide a fake activity name/informations to better differentiate it from the wireframe (Which contains those kinds of placeholders). The same goes with the image placeholders with shapes displayed.
- As already suggested in M1. please link the different page together with the actions of the buttons (not graded, but makes it way more usable)
