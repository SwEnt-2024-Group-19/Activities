...
class ActivityDetailsScreenAndroidTest {
  ...
  fun activityTitle_isDisplayed() {
    ...
    `when`(mockViewModel.selectedActivity).thenReturn(MutableStateFlow<Activity?>(activity).asStateFlow())
    `when`(mockProfileViewModel.userState).thenReturn(MutableStateFlow(null).asStateFlow())
