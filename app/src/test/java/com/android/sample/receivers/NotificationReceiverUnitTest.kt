...
class NotificationReceiverUnitTest {
    ...
    fun `onReceive should create and send notification with valid intent`() {
        ...
        assert(shadowNotification.contentTitle == notificationTitle) { "Notification title does not match" }
        assert(shadowNotification.contentText == "Reminder: $activityName") { "Notification content does not match" }
    }
